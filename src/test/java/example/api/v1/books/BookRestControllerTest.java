...
public class BookRestControllerTest extends BaseTestRestController {
	...
	public void getAllBooks() throws Exception {
		mockMvc.perform(get("/v1/books")).andExpect(status().isOk()).andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$.data", hasSize(1))).andExpect(jsonPath("$.data[0].id", greaterThan(0)))
	...
	public void getAnExistingBook() throws Exception {
		mockMvc.perform(get("/v1/books/" + book.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(contentType)).andExpect(jsonPath("$.data.id", greaterThan(0)))
